...
- G2 (live refs — ALL file types, not just Markdown): `grep -rn "<basename>" <repo> --exclude-dir={node_modules,dist,.nx,.git} | grep -v worktrees | grep -v changes/archive` → repoint EVERY live ref. Do NOT restrict to `--include=*.md`: live references live in configs, CI workflows, and source comments too (e.g. a governance doc cited from a lint config, a CI workflow, or an ESLint config file).
